...
class Group(nn.Module):  # FPS + KNN
    ...
    def forward(self, xyz):
        ...
        center, _ = sample_farthest_points(xyz, self.num_group) # [B, npoint, 3]  [B, npoint]
        idx = knn_point(self.group_size, xyz, center) # [B, npoint, k]
...
class MaskTransformer(nn.Module):
    ...
    def forward(self, neighborhood, center, pos, test=False, feature_dict=None, mix_rate=1.0):
        # generate mask
        if self.training:
        ...
        # test-time DG shifting
        if test:
            test_pc_feature = group_input_tokens[:, feature_len: 2*feature_len, :]
            test_pc_global = torch.max(test_pc_feature, dim=1)[0].unsqueeze(1)
            B, G, C = test_pc_feature.shape
            D = len(feature_dict['dataset'])
            test_local_norm = F.normalize(test_pc_feature, dim=-1)
            test_global_norm = F.normalize(test_pc_global, dim=-1)

            local_simi_all = torch.empty([B, G, D], device=test_pc_feature.device)
            global_simi_all = torch.empty([B, D], device=test_pc_feature.device)
            for idx in range(D):
                train_local_prototype = feature_dict['local_prototype'][idx]
                train_global_prototype = feature_dict['global_prototype'][idx]
                train_local_norm = F.normalize(train_local_prototype, dim=-1).repeat(B, 1, 1)
                train_global_norm = F.normalize(train_global_prototype.unsqueeze(0), dim=-1).repeat(B, 1, 1)
                
                
                local_simi = torch.mean(torch.matmul(test_local_norm, train_local_norm.transpose(2,1)), dim=-1)
                global_simi = torch.matmul(test_global_norm, train_global_norm.transpose(2,1)).squeeze()
                local_simi_all[:, :, idx] = F.softmax(local_simi, dim=-1)
                global_simi_all[:, idx] = global_simi
            global_simi_all = F.softmax(global_simi_all, dim=-1)
            
            test_pc_feature_mixed = torch.zeros_like(test_pc_feature, device=test_pc_feature.device)
            for idx in range(D):
                local_prototype_mix = (1-global_simi_all[:, idx].view(B,1,1))*(1-local_simi_all[:, :, idx].unsqueeze(-1))*feature_dict['local_prototype'][idx].unsqueeze(0).repeat(B, 1, 1)
                local_feature_mix = global_simi_all[:, idx].view(B,1,1)*local_simi_all[:, :, idx].unsqueeze(-1)*test_pc_feature
                test_pc_feature_mixed += (local_prototype_mix + local_feature_mix)/G
            test_pc_feature_mixed /= D

            group_feature_front = group_input_tokens[:, : feature_len, :]
            group_feature_back = group_input_tokens[:, 2*feature_len: , :]
            group_input_tokens = torch.cat([group_feature_front, test_pc_feature_mixed, group_feature_back], dim=1)
...
class DGPIC(nn.Module):
    def __init__(self, config):
        super().__init__()
        print_log(f'[DGPIC] ', logger ='DGPIC')
        self.config = config
        self.trans_dim = config.transformer_config.trans_dim
        ...
        print_log(f'[DGPIC] divide point cloud into G{self.num_group} x S{self.group_size} points ...', logger ='DGPIC')
    ...
    def get_patch(self, pc1, pc2, target1, target2):
        pc1_center, pc1_center_idx = sample_farthest_points(pc1, self.num_group)
        pc2_center, pc2_center_idx = sample_farthest_points(pc2, self.num_group)

        pc1_neighborhood_idx = knn_point(self.group_size, pc1, pc1_center)
        pc2_neighborhood_idx = knn_point(self.group_size, pc2, pc2_center)
        ...
        target1_neighborhood_idx = knn_point(self.group_size, target1, target1_center)
        target2_neighborhood_idx = knn_point(self.group_size, target2, target2_center)
    ...
    def forward(self, pc1, pc2, target1, target2, test=False, feature_dict=None, mix_rate=1.0, **kwargs):
        ...
        x, mask, latent_feature = self.MAE_encoder(neighborhood, center, self.pos_sincos, test, feature_dict, mix_rate)
        B, _, C = x.shape
        ...
        return latent_feature, rebuild_points, loss